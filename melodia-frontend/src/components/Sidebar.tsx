...
export function Sidebar({ currentView, onNavigate, onShowAuth }: Props) {
  ...
  return (
    <aside className="w-60 bg-gray-950 border-r border-gray-800 flex flex-col h-full flex-shrink-0">
      ...
      <div className="p-3 border-t border-gray-800">
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <button onClick={signOut} className="text-gray-400 hover:text-rose-400 transition-colors p-1.5 rounded-lg hover:bg-gray-800">
              <LogOut size={16} />
            </button>
          </div>
        ) : (
          <button
            onClick={onShowAuth}
            className="w-full bg-rose-500 hover:bg-rose-600 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors"
          >
            Sign In
          </button>
        )}
